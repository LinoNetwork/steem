...
#define STEEMIT_INIT_PUBLIC_KEY_STR             "STX8hfpbPWX551MLzDYdr8XCG41xDet9LPzK8G6qyEAwndHAKpzP1"
#define STEEMIT_CHAIN_ID                        (fc::sha256::hash("linotest"))
#define VESTS_SYMBOL  (uint64_t(6) | (uint64_t('V') << 8) | (uint64_t('E') << 16) | (uint64_t('S') << 24) | (uint64_t('T') << 32) | (uint64_t('S') << 40)) ///< VESTS with 6 digits of precision
...
#define STEEMIT_MAX_CASHOUT_WINDOW_SECONDS      (60*60*24*18250) /// 2 weeks
...
#define STEEMIT_INIT_SBD_SUPPLY                 int64_t(1000000ll)
